...
commpact_status_t ocall_store_sealed_keys( const uint8_t* sealed_data, uint32_t len ) {
	FILE* fp;
	fp = fopen( STORE_FILENAME, "wb" );
	if (fp == NULL) {
		return CP_CANT_OPEN_FILE;
	}
...
commpact_status_t ocall_load_sealed_keys( uint8_t* sealed_data, uint32_t len ) {
	FILE* fp;
...
int SGX_CDECL main(int argc, char* argv[]) {
	...
	commpact_status_t 	j_status   = CP_SUCCESS;
